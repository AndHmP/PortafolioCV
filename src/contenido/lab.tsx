import type { PiezaLab } from '@/tipos';

import '@/lab/estilos/utilidades-heredadas.css';

/* Generado por scripts/generar-registro-lab.mjs — no editar a mano. */

import Dia01 from '@/lab/animaciones/Dia01';
import Dia02 from '@/lab/animaciones/Dia02';
import Dia03 from '@/lab/animaciones/Dia03';
import Dia04 from '@/lab/animaciones/Dia04';
import Dia05 from '@/lab/animaciones/Dia05';
import Dia06 from '@/lab/animaciones/Dia06';
import Dia07 from '@/lab/animaciones/Dia07';
import Dia08 from '@/lab/animaciones/Dia08';
import Dia09 from '@/lab/animaciones/Dia09';
import Dia10 from '@/lab/animaciones/Dia10';
import Dia11 from '@/lab/animaciones/Dia11';
import Dia12 from '@/lab/animaciones/Dia12';
import Dia13 from '@/lab/animaciones/Dia13';
import Dia14 from '@/lab/animaciones/Dia14';
import Dia15 from '@/lab/animaciones/Dia15';
import Dia16 from '@/lab/animaciones/Dia16';
import Dia17 from '@/lab/animaciones/Dia17';
import Dia18 from '@/lab/animaciones/Dia18';
import Dia19 from '@/lab/animaciones/Dia19';
import Dia20 from '@/lab/animaciones/Dia20';
import Dia21 from '@/lab/animaciones/Dia21';
import Dia22 from '@/lab/animaciones/Dia22';
import Dia23 from '@/lab/animaciones/Dia23';
import Dia24 from '@/lab/animaciones/Dia24';
import Dia25 from '@/lab/animaciones/Dia25';
import Dia26 from '@/lab/animaciones/Dia26';
import Dia27 from '@/lab/animaciones/Dia27';
import Dia28 from '@/lab/animaciones/Dia28';
import Dia29 from '@/lab/animaciones/Dia29';
import Dia30 from '@/lab/animaciones/Dia30';
import Dia31 from '@/lab/animaciones/Dia31';
import Dia32 from '@/lab/animaciones/Dia32';
import Dia33 from '@/lab/animaciones/Dia33';
import Dia34 from '@/lab/animaciones/Dia34';
import Dia35 from '@/lab/animaciones/Dia35';
import Dia36 from '@/lab/animaciones/Dia36';
import Dia37 from '@/lab/animaciones/Dia37';
import Dia38 from '@/lab/animaciones/Dia38';
import Dia39 from '@/lab/animaciones/Dia39';
import Dia40 from '@/lab/animaciones/Dia40';
import Dia41 from '@/lab/animaciones/Dia41';
import Dia42 from '@/lab/animaciones/Dia42';
import Dia43 from '@/lab/animaciones/Dia43';
import Dia44 from '@/lab/animaciones/Dia44';
import Dia45 from '@/lab/animaciones/Dia45';
import Dia46 from '@/lab/animaciones/Dia46';
import Dia47 from '@/lab/animaciones/Dia47';
import Dia48 from '@/lab/animaciones/Dia48';
import Dia49 from '@/lab/animaciones/Dia49';
import Dia50 from '@/lab/animaciones/Dia50';
import Dia51 from '@/lab/animaciones/Dia51';
import Dia52 from '@/lab/animaciones/Dia52';
import Dia53 from '@/lab/animaciones/Dia53';
import Dia54 from '@/lab/animaciones/Dia54';
import Dia55 from '@/lab/animaciones/Dia55';
import Dia56 from '@/lab/animaciones/Dia56';
import Dia57 from '@/lab/animaciones/Dia57';
import Dia58 from '@/lab/animaciones/Dia58';
import Dia59 from '@/lab/animaciones/Dia59';
import Dia60 from '@/lab/animaciones/Dia60';
import Dia61 from '@/lab/animaciones/Dia61';
import Dia62 from '@/lab/animaciones/Dia62';
import Dia63 from '@/lab/animaciones/Dia63';
import Dia64 from '@/lab/animaciones/Dia64';
import Dia65 from '@/lab/animaciones/Dia65';
import Dia66 from '@/lab/animaciones/Dia66';
import Dia67 from '@/lab/animaciones/Dia67';
import Dia68 from '@/lab/animaciones/Dia68';
import Dia69 from '@/lab/animaciones/Dia69';
import Dia70 from '@/lab/animaciones/Dia70';
import Dia71 from '@/lab/animaciones/Dia71';
import Dia72 from '@/lab/animaciones/Dia72';
import Dia73 from '@/lab/animaciones/Dia73';
import Dia74 from '@/lab/animaciones/Dia74';
import Dia75 from '@/lab/animaciones/Dia75';
import Dia76 from '@/lab/animaciones/Dia76';
import Dia77 from '@/lab/animaciones/Dia77';
import Dia78 from '@/lab/animaciones/Dia78';
import Dia79 from '@/lab/animaciones/Dia79';
import Dia80 from '@/lab/animaciones/Dia80';
import Dia81 from '@/lab/animaciones/Dia81';
import Dia82 from '@/lab/animaciones/Dia82';
import Dia83 from '@/lab/animaciones/Dia83';
import Dia84 from '@/lab/animaciones/Dia84';
import Dia85 from '@/lab/animaciones/Dia85';
import Dia86 from '@/lab/animaciones/Dia86';
import Dia87 from '@/lab/animaciones/Dia87';
import Dia88 from '@/lab/animaciones/Dia88';
import Dia89 from '@/lab/animaciones/Dia89';
import Dia90 from '@/lab/animaciones/Dia90';
import Dia91 from '@/lab/animaciones/Dia91';
import Dia92 from '@/lab/animaciones/Dia92';
import Dia93 from '@/lab/animaciones/Dia93';
import Dia94 from '@/lab/animaciones/Dia94';
import Dia95 from '@/lab/animaciones/Dia95';
import Dia96 from '@/lab/animaciones/Dia96';
import Dia97 from '@/lab/animaciones/Dia97';
import Dia98 from '@/lab/animaciones/Dia98';
import Dia99 from '@/lab/animaciones/Dia99';
import Dia100 from '@/lab/animaciones/Dia100';

import InputType01 from '@/lab/inputs/InputType01';
import InputType02 from '@/lab/inputs/InputType02';
import InputType03 from '@/lab/inputs/InputType03';
import InputType04 from '@/lab/inputs/InputType04';
import InputType05 from '@/lab/inputs/InputType05';
import InputType06 from '@/lab/inputs/InputType06';

import ButtonType01 from '@/lab/botones/ButtonType01';
import ButtonType02 from '@/lab/botones/ButtonType02';
import LoginDemo from '@/lab/login/LoginDemo';

import Carrusel from '@/lab/cards/Carrusel';
import CardTipo01 from '@/lab/cards/CardTipo01';
import CardTipo02 from '@/lab/cards/CardTipo02';
import CardTipo03 from '@/lab/cards/CardTipo03';
import CardTipo04 from '@/lab/cards/CardTipo04';
import CardTipo05 from '@/lab/cards/CardTipo05';
import CardTipo06 from '@/lab/cards/CardTipo06';
import CardTipo07 from '@/lab/cards/CardTipo07';
import CardTipo08 from '@/lab/cards/CardTipo08';
import { coleccionables, destinos, empleados, perfilesRed, productos, promociones, zapatillas } from '@/lab/cards/datos';

export const piezasLab: PiezaLab[] = [
  {
    slug: 'animacion-01',
    titulo: { es: "Día 01 · 100 Days CSS", en: "Day 01 · 100 Days CSS" },
    seccion: '100-dias-css',
    categoria: 'animaciones',
    tecnica: { es: "Degradados, sombras compuestas, posicionamiento absoluto.", en: "Gradients, layered shadows, absolute positioning." },
    componente: <Dia01 />,
    fondoFijo: 'oscuro',
  },
  {
    slug: 'animacion-02',
    titulo: { es: "Día 02 · Icono de menú", en: "Day 02 · Menu Icon" },
    seccion: '100-dias-css',
    categoria: 'animaciones',
    tecnica: { es: "Sombras compuestas, transiciones.", en: "Layered shadows, transitions." },
    componente: <Dia02 />,
    fondoFijo: 'oscuro',
  },
  {
    slug: 'animacion-03',
    titulo: { es: "Día 03 · La pirámide", en: "Day 03 · The Pyramide" },
    seccion: '100-dias-css',
    categoria: 'animaciones',
    tecnica: { es: "Animaciones por fotogramas, recortes con clip-path, posicionamiento absoluto.", en: "Keyframe animations, clip-path masking, absolute positioning." },
    componente: <Dia03 />,
    fondoFijo: 'oscuro',
  },
  {
    slug: 'animacion-04',
    titulo: { es: "Día 04 · Cargando", en: "Day 04 · Loading" },
    seccion: '100-dias-css',
    categoria: 'animaciones',
    tecnica: { es: "Animaciones por fotogramas, sombras compuestas, posicionamiento absoluto.", en: "Keyframe animations, layered shadows, absolute positioning." },
    componente: <Dia04 />,
    fondoFijo: 'oscuro',
  },
  {
    slug: 'animacion-05',
    titulo: { es: "Día 05 · Estadística", en: "Day 05 · Statistic" },
    seccion: '100-dias-css',
    categoria: 'animaciones',
    tecnica: { es: "Recortes con clip-path, sombras compuestas, CSS Grid.", en: "Clip-path masking, layered shadows, CSS Grid." },
    componente: <Dia05 />,
    fondoFijo: 'oscuro',
  },
  {
    slug: 'animacion-06',
    titulo: { es: "Día 06 · Perfil", en: "Day 06 · Profile" },
    seccion: '100-dias-css',
    categoria: 'animaciones',
    tecnica: { es: "Recortes con clip-path, sombras compuestas, CSS Grid.", en: "Clip-path masking, layered shadows, CSS Grid." },
    componente: <Dia06 />,
    fondoFijo: 'oscuro',
  },
  {
    slug: 'animacion-07',
    titulo: { es: "Día 07 · Notificaciones, búsqueda y menú", en: "Day 07 · Notifications, Search and Menu" },
    seccion: '100-dias-css',
    categoria: 'animaciones',
    tecnica: { es: "Sombras compuestas, estados de interacción, transiciones.", en: "Layered shadows, interaction states, transitions." },
    componente: <Dia07 />,
    fondoFijo: 'oscuro',
  },
  {
    slug: 'animacion-08',
    titulo: { es: "Día 08 · Metaballs", en: "Day 08 · Metaballs" },
    seccion: '100-dias-css',
    categoria: 'animaciones',
    tecnica: { es: "Animaciones por fotogramas, generación de reglas con Sass, filtros CSS.", en: "Keyframe animations, rule generation with Sass, CSS filters." },
    componente: <Dia08 />,
    fondoFijo: 'oscuro',
  },
  {
    slug: 'animacion-09',
    titulo: { es: "Día 09 · Noche lluviosa", en: "Day 09 · Rainy Night" },
    seccion: '100-dias-css',
    categoria: 'animaciones',
    tecnica: { es: "Animaciones por fotogramas, recortes con clip-path, generación de reglas con Sass.", en: "Keyframe animations, clip-path masking, rule generation with Sass." },
    componente: <Dia09 />,
    fondoFijo: 'oscuro',
  },
  {
    slug: 'animacion-10',
    titulo: { es: "Día 10 · Reloj", en: "Day 10 · Watch" },
    seccion: '100-dias-css',
    categoria: 'animaciones',
    tecnica: { es: "Animaciones por fotogramas, generación de reglas con Sass, trazado de SVG animado.", en: "Keyframe animations, rule generation with Sass, animated SVG stroke." },
    componente: <Dia10 />,
    fondoFijo: 'oscuro',
  },
  {
    slug: 'animacion-11',
    titulo: { es: "Día 11 · Botas de senderismo", en: "Day 11 · Walking Boots" },
    seccion: '100-dias-css',
    categoria: 'animaciones',
    tecnica: { es: "Animaciones por fotogramas, generación de reglas con Sass, posicionamiento absoluto.", en: "Keyframe animations, rule generation with Sass, absolute positioning." },
    componente: <Dia11 />,
    fondoFijo: 'oscuro',
  },
  {
    slug: 'animacion-12',
    titulo: { es: "Día 12 · Tooltip", en: "Day 12 · Tooltip" },
    seccion: '100-dias-css',
    categoria: 'animaciones',
    tecnica: { es: "Recortes con clip-path, estados de interacción, transiciones.", en: "Clip-path masking, interaction states, transitions." },
    componente: <Dia12 />,
    fondoFijo: 'oscuro',
  },
  {
    slug: 'animacion-13',
    titulo: { es: "Día 13 · Galería de usuarios", en: "Day 13 · User Gallery" },
    seccion: '100-dias-css',
    categoria: 'animaciones',
    tecnica: { es: "Filtros CSS, sombras compuestas, CSS Grid.", en: "CSS filters, layered shadows, CSS Grid." },
    componente: <Dia13 />,
    fondoFijo: 'oscuro',
  },
  {
    slug: 'animacion-14',
    titulo: { es: "Día 14 · Byciclopter", en: "Day 14 · Byciclopter" },
    seccion: '100-dias-css',
    categoria: 'animaciones',
    tecnica: { es: "Transformaciones 3D, animaciones por fotogramas, generación de reglas con Sass.", en: "3D transforms, keyframe animations, rule generation with Sass." },
    componente: <Dia14 />,
    fondoFijo: 'oscuro',
  },
  {
    slug: 'animacion-15',
    titulo: { es: "Día 15 · Subir archivo", en: "Day 15 · Upload" },
    seccion: '100-dias-css',
    categoria: 'animaciones',
    tecnica: { es: "Animaciones por fotogramas, degradados, sombras compuestas.", en: "Keyframe animations, gradients, layered shadows." },
    componente: <Dia15 />,
    fondoFijo: 'oscuro',
  },
  {
    slug: 'animacion-16',
    titulo: { es: "Día 16 · Blobby", en: "Day 16 · Blobby" },
    seccion: '100-dias-css',
    categoria: 'animaciones',
    tecnica: { es: "Animaciones por fotogramas, recortes con clip-path, generación de reglas con Sass.", en: "Keyframe animations, clip-path masking, rule generation with Sass." },
    componente: <Dia16 />,
    fondoFijo: 'oscuro',
  },
  {
    slug: 'animacion-17',
    titulo: { es: "Día 17 · Triángulo de Penrose", en: "Day 17 · Penrose Triangle" },
    seccion: '100-dias-css',
    categoria: 'animaciones',
    tecnica: { es: "Recortes con clip-path, degradados, sombras compuestas.", en: "Clip-path masking, gradients, layered shadows." },
    componente: <Dia17 />,
    fondoFijo: 'oscuro',
  },
  {
    slug: 'animacion-18',
    titulo: { es: "Día 18 · Elástico", en: "Day 18 · Elastic" },
    seccion: '100-dias-css',
    categoria: 'animaciones',
    tecnica: { es: "Animaciones por fotogramas, degradados, sombras compuestas.", en: "Keyframe animations, gradients, layered shadows." },
    componente: <Dia18 />,
    fondoFijo: 'oscuro',
  },
  {
    slug: 'animacion-19',
    titulo: { es: "Día 19 · Carrusel con botones de radio", en: "Day 19 · Slider with Radio Buttons" },
    seccion: '100-dias-css',
    categoria: 'animaciones',
    tecnica: { es: "Generación de reglas con Sass, transiciones, posicionamiento absoluto.", en: "Rule generation with Sass, transitions, absolute positioning." },
    componente: <Dia19 />,
    fondoFijo: 'oscuro',
  },
  {
    slug: 'animacion-20',
    titulo: { es: "Día 20 · Enviar correo", en: "Day 20 · Send Mail" },
    seccion: '100-dias-css',
    categoria: 'animaciones',
    tecnica: { es: "Animaciones por fotogramas, trazado de SVG animado, estados de interacción.", en: "Keyframe animations, animated SVG stroke, interaction states." },
    componente: <Dia20 />,
    fondoFijo: 'oscuro',
  },
  {
    slug: 'animacion-21',
    titulo: { es: "Día 21 · Pac-Man", en: "Day 21 · Pac-Man" },
    seccion: '100-dias-css',
    categoria: 'animaciones',
    tecnica: { es: "Animaciones por fotogramas, recortes con clip-path, trazado de SVG animado.", en: "Keyframe animations, clip-path masking, animated SVG stroke." },
    componente: <Dia21 />,
    fondoFijo: 'oscuro',
  },
  {
    slug: 'animacion-22',
    titulo: { es: "Día 22 · Pulsera de actividad", en: "Day 22 · Fitness Tracker" },
    seccion: '100-dias-css',
    categoria: 'animaciones',
    tecnica: { es: "Animaciones por fotogramas, trazado de SVG animado, CSS Grid.", en: "Keyframe animations, animated SVG stroke, CSS Grid." },
    componente: <Dia22 />,
    fondoFijo: 'oscuro',
  },
  {
    slug: 'animacion-23',
    titulo: { es: "Día 23 · Tipografía animada", en: "Day 23 · Animated Typography" },
    seccion: '100-dias-css',
    categoria: 'animaciones',
    tecnica: { es: "Animaciones por fotogramas, recortes con clip-path, formas circulares.", en: "Keyframe animations, clip-path masking, circular shapes." },
    componente: <Dia23 />,
    fondoFijo: 'oscuro',
  },
  {
    slug: 'animacion-24',
    titulo: { es: "Día 24 · Botón", en: "Day 24 · Button" },
    seccion: '100-dias-css',
    categoria: 'animaciones',
    tecnica: { es: "Animaciones por fotogramas, trazado de SVG animado, estados de interacción.", en: "Keyframe animations, animated SVG stroke, interaction states." },
    componente: <Dia24 />,
    fondoFijo: 'oscuro',
  },
  {
    slug: 'animacion-25',
    titulo: { es: "Día 25 · Marcador de mapa", en: "Day 25 · Map Marker" },
    seccion: '100-dias-css',
    categoria: 'animaciones',
    tecnica: { es: "Transformaciones 3D, animaciones por fotogramas, sombras compuestas.", en: "3D transforms, keyframe animations, layered shadows." },
    componente: <Dia25 />,
    fondoFijo: 'oscuro',
  },
  {
    slug: 'animacion-26',
    titulo: { es: "Día 26 · Modal motivacional", en: "Day 26 · Motivational Modal" },
    seccion: '100-dias-css',
    categoria: 'animaciones',
    tecnica: { es: "Animaciones por fotogramas, recortes con clip-path, generación de reglas con Sass.", en: "Keyframe animations, clip-path masking, rule generation with Sass." },
    componente: <Dia26 />,
    fondoFijo: 'oscuro',
  },
  {
    slug: 'animacion-27',
    titulo: { es: "Día 27 · Checklist", en: "Day 27 · Checklist" },
    seccion: '100-dias-css',
    categoria: 'animaciones',
    tecnica: { es: "Recortes con clip-path, trazado de SVG animado, degradados.", en: "Clip-path masking, animated SVG stroke, gradients." },
    componente: <Dia27 />,
    fondoFijo: 'oscuro',
  },
  {
    slug: 'animacion-28',
    titulo: { es: "Día 28 · Ring Ring", en: "Day 28 · Ring Ring" },
    seccion: '100-dias-css',
    categoria: 'animaciones',
    tecnica: { es: "Animaciones por fotogramas, posicionamiento absoluto.", en: "Keyframe animations, absolute positioning." },
    componente: <Dia28 />,
    fondoFijo: 'oscuro',
  },
  {
    slug: 'animacion-29',
    titulo: { es: "Día 29 · Campo de búsqueda", en: "Day 29 · Search Field" },
    seccion: '100-dias-css',
    categoria: 'animaciones',
    tecnica: { es: "Recortes con clip-path, sombras compuestas, estados de interacción.", en: "Clip-path masking, layered shadows, interaction states." },
    componente: <Dia29 />,
    fondoFijo: 'oscuro',
  },
  {
    slug: 'animacion-30',
    titulo: { es: "Día 30 · Línea al azar", en: "Day 30 · Random Line" },
    seccion: '100-dias-css',
    categoria: 'animaciones',
    tecnica: { es: "Animaciones por fotogramas, trazado de SVG animado, posicionamiento absoluto.", en: "Keyframe animations, animated SVG stroke, absolute positioning." },
    componente: <Dia30 />,
    fondoFijo: 'oscuro',
  },
  {
    slug: 'animacion-31',
    titulo: { es: "Día 31 · Onda de péndulos", en: "Day 31 · Pendulum Wave" },
    seccion: '100-dias-css',
    categoria: 'animaciones',
    tecnica: { es: "Animaciones por fotogramas, generación de reglas con Sass, formas circulares.", en: "Keyframe animations, rule generation with Sass, circular shapes." },
    componente: <Dia31 />,
    fondoFijo: 'oscuro',
  },
  {
    slug: 'animacion-32',
    titulo: { es: "Día 32 · Contador", en: "Day 32 · Counter" },
    seccion: '100-dias-css',
    categoria: 'animaciones',
    tecnica: { es: "Animaciones por fotogramas, degradados, sombras compuestas.", en: "Keyframe animations, gradients, layered shadows." },
    componente: <Dia32 />,
    fondoFijo: 'oscuro',
  },
  {
    slug: 'animacion-33',
    titulo: { es: "Día 33 · Día soleado", en: "Day 33 · Sunny Day" },
    seccion: '100-dias-css',
    categoria: 'animaciones',
    tecnica: { es: "Animaciones por fotogramas, posicionamiento absoluto, formas circulares.", en: "Keyframe animations, absolute positioning, circular shapes." },
    componente: <Dia33 />,
    fondoFijo: 'oscuro',
  },
  {
    slug: 'animacion-34',
    titulo: { es: "Día 34 · Hello", en: "Day 34 · Hello" },
    seccion: '100-dias-css',
    categoria: 'animaciones',
    tecnica: { es: "Animaciones por fotogramas, trazado de SVG animado.", en: "Keyframe animations, animated SVG stroke." },
    componente: <Dia34 />,
    fondoFijo: 'oscuro',
  },
  {
    slug: 'animacion-35',
    titulo: { es: "Día 35 · Círculo de carga", en: "Day 35 · Loading Circle" },
    seccion: '100-dias-css',
    categoria: 'animaciones',
    tecnica: { es: "Animaciones por fotogramas, trazado de SVG animado.", en: "Keyframe animations, animated SVG stroke." },
    componente: <Dia35 />,
    fondoFijo: 'oscuro',
  },
  {
    slug: 'animacion-36',
    titulo: { es: "Día 36 · Pestañas", en: "Day 36 · Tabs" },
    seccion: '100-dias-css',
    categoria: 'animaciones',
    tecnica: { es: "Sombras compuestas, estados de interacción, transiciones.", en: "Layered shadows, interaction states, transitions." },
    componente: <Dia36 />,
    fondoFijo: 'oscuro',
  },
  {
    slug: 'animacion-37',
    titulo: { es: "Día 37 · Carrusel", en: "Day 37 · Carousel" },
    seccion: '100-dias-css',
    categoria: 'animaciones',
    tecnica: { es: "Transformaciones 3D, animaciones por fotogramas, sombras compuestas.", en: "3D transforms, keyframe animations, layered shadows." },
    componente: <Dia37 />,
    fondoFijo: 'oscuro',
  },
  {
    slug: 'animacion-38',
    titulo: { es: "Día 38 · Transición de logo", en: "Day 38 · Logo Transition" },
    seccion: '100-dias-css',
    categoria: 'animaciones',
    tecnica: { es: "Transiciones, posicionamiento absoluto.", en: "Transitions, absolute positioning." },
    componente: <Dia38 />,
    fondoFijo: 'oscuro',
  },
  {
    slug: 'animacion-39',
    titulo: { es: "Día 39 · Menú hamburguesa", en: "Day 39 · Menu Toggle" },
    seccion: '100-dias-css',
    categoria: 'animaciones',
    tecnica: { es: "Animaciones por fotogramas, estados de interacción, transiciones.", en: "Keyframe animations, interaction states, transitions." },
    componente: <Dia39 />,
    fondoFijo: 'oscuro',
  },
  {
    slug: 'animacion-40',
    titulo: { es: "Día 40 · Galería", en: "Day 40 · Gallery" },
    seccion: '100-dias-css',
    categoria: 'animaciones',
    tecnica: { es: "Animaciones por fotogramas, CSS Grid, transiciones.", en: "Keyframe animations, CSS Grid, transitions." },
    componente: <Dia40 />,
    fondoFijo: 'oscuro',
  },
  {
    slug: 'animacion-41',
    titulo: { es: "Día 41 · Modal de error", en: "Day 41 · Error Modal" },
    seccion: '100-dias-css',
    categoria: 'animaciones',
    tecnica: { es: "Animaciones por fotogramas, sombras compuestas, estados de interacción.", en: "Keyframe animations, layered shadows, interaction states." },
    componente: <Dia41 />,
    fondoFijo: 'oscuro',
  },
  {
    slug: 'animacion-42',
    titulo: { es: "Día 42 · Estrellas", en: "Day 42 · Stars" },
    seccion: '100-dias-css',
    categoria: 'animaciones',
    tecnica: { es: "Animaciones por fotogramas, degradados, posicionamiento absoluto.", en: "Keyframe animations, gradients, absolute positioning." },
    componente: <Dia42 />,
    fondoFijo: 'oscuro',
  },
  {
    slug: 'animacion-43',
    titulo: { es: "Día 43 · Bombilla", en: "Day 43 · Lightbulb" },
    seccion: '100-dias-css',
    categoria: 'animaciones',
    tecnica: { es: "Sombras compuestas, estados de interacción, transiciones.", en: "Layered shadows, interaction states, transitions." },
    componente: <Dia43 />,
    fondoFijo: 'oscuro',
  },
  {
    slug: 'animacion-44',
    titulo: { es: "Día 44 · Pirámide retorcida", en: "Day 44 · Twisted Pyramid" },
    seccion: '100-dias-css',
    categoria: 'animaciones',
    tecnica: { es: "Animaciones por fotogramas, generación de reglas con Sass, estados de interacción.", en: "Keyframe animations, rule generation with Sass, interaction states." },
    componente: <Dia44 />,
    fondoFijo: 'oscuro',
  },
  {
    slug: 'animacion-45',
    titulo: { es: "Día 45 · Botón", en: "Day 45 · Button" },
    seccion: '100-dias-css',
    categoria: 'animaciones',
    tecnica: { es: "Trazado de SVG animado, estados de interacción, transiciones.", en: "Animated SVG stroke, interaction states, transitions." },
    componente: <Dia45 />,
    fondoFijo: 'oscuro',
  },
  {
    slug: 'animacion-46',
    titulo: { es: "Día 46 · Círculos de iris", en: "Day 46 · Iris Circles" },
    seccion: '100-dias-css',
    categoria: 'animaciones',
    tecnica: { es: "Transformaciones 3D, animaciones por fotogramas, generación de reglas con Sass.", en: "3D transforms, keyframe animations, rule generation with Sass." },
    componente: <Dia46 />,
    fondoFijo: 'oscuro',
  },
  {
    slug: 'animacion-47',
    titulo: { es: "Día 47 · Pixel art", en: "Day 47 · Pixel Art" },
    seccion: '100-dias-css',
    categoria: 'animaciones',
    tecnica: { es: "CSS Grid, estados de interacción.", en: "CSS Grid, interaction states." },
    componente: <Dia47 />,
    fondoFijo: 'oscuro',
  },
  {
    slug: 'animacion-48',
    titulo: { es: "Día 48 · Cubo 3D", en: "Day 48 · 3D Cube" },
    seccion: '100-dias-css',
    categoria: 'animaciones',
    tecnica: { es: "Transformaciones 3D, estados de interacción, transiciones.", en: "3D transforms, interaction states, transitions." },
    componente: <Dia48 />,
    fondoFijo: 'oscuro',
  },
  {
    slug: 'animacion-49',
    titulo: { es: "Día 49 · CSS Follow", en: "Day 49 · CSS Follow" },
    seccion: '100-dias-css',
    categoria: 'animaciones',
    tecnica: { es: "Animaciones por fotogramas, CSS Grid, transiciones.", en: "Keyframe animations, CSS Grid, transitions." },
    componente: <Dia49 />,
    fondoFijo: 'oscuro',
  },
  {
    slug: 'animacion-50',
    titulo: { es: "Día 50 · Huevo de pascua", en: "Day 50 · Easter Egg" },
    seccion: '100-dias-css',
    categoria: 'animaciones',
    tecnica: { es: "Animaciones por fotogramas, posicionamiento absoluto.", en: "Keyframe animations, absolute positioning." },
    componente: <Dia50 />,
    fondoFijo: 'oscuro',
  },
  {
    slug: 'animacion-51',
    titulo: { es: "Día 51 · Contador CSS", en: "Day 51 · CSS Counter" },
    seccion: '100-dias-css',
    categoria: 'animaciones',
    tecnica: { es: "Animaciones por fotogramas, sombras compuestas, transiciones.", en: "Keyframe animations, layered shadows, transitions." },
    componente: <Dia51 />,
    fondoFijo: 'oscuro',
  },
  {
    slug: 'animacion-52',
    titulo: { es: "Día 52 · Onda de puntos", en: "Day 52 · Dot Wave" },
    seccion: '100-dias-css',
    categoria: 'animaciones',
    tecnica: { es: "Animaciones por fotogramas, posicionamiento absoluto, formas circulares.", en: "Keyframe animations, absolute positioning, circular shapes." },
    componente: <Dia52 />,
    fondoFijo: 'oscuro',
  },
  {
    slug: 'animacion-53',
    titulo: { es: "Día 53 · Control deslizante", en: "Day 53 · Range Slider" },
    seccion: '100-dias-css',
    categoria: 'animaciones',
    tecnica: { es: "Sombras compuestas, posicionamiento absoluto, formas circulares.", en: "Layered shadows, absolute positioning, circular shapes." },
    componente: <Dia53 />,
    fondoFijo: 'oscuro',
  },
  {
    slug: 'animacion-54',
    titulo: { es: "Día 54 · Olas", en: "Day 54 · Waves" },
    seccion: '100-dias-css',
    categoria: 'animaciones',
    tecnica: { es: "Animaciones por fotogramas, posicionamiento absoluto, formas circulares.", en: "Keyframe animations, absolute positioning, circular shapes." },
    componente: <Dia54 />,
    fondoFijo: 'oscuro',
  },
  {
    slug: 'animacion-55',
    titulo: { es: "Día 55 · Termostato", en: "Day 55 · Thermostat" },
    seccion: '100-dias-css',
    categoria: 'animaciones',
    tecnica: { es: "Degradados, sombras compuestas, estados de interacción.", en: "Gradients, layered shadows, interaction states." },
    componente: <Dia55 />,
    fondoFijo: 'oscuro',
  },
  {
    slug: 'animacion-56',
    titulo: { es: "Día 56 · Flor", en: "Day 56 · Flower" },
    seccion: '100-dias-css',
    categoria: 'animaciones',
    tecnica: { es: "Animaciones por fotogramas, generación de reglas con Sass, sombras compuestas.", en: "Keyframe animations, rule generation with Sass, layered shadows." },
    componente: <Dia56 />,
    fondoFijo: 'oscuro',
  },
  {
    slug: 'animacion-57',
    titulo: { es: "Día 57 · Icosaedro", en: "Day 57 · Icosahedron" },
    seccion: '100-dias-css',
    categoria: 'animaciones',
    tecnica: { es: "Transformaciones 3D, estados de interacción, transiciones.", en: "3D transforms, interaction states, transitions." },
    componente: <Dia57 />,
    fondoFijo: 'oscuro',
  },
  {
    slug: 'animacion-58',
    titulo: { es: "Día 58 · Esfera 3D", en: "Day 58 · 3D Sphere" },
    seccion: '100-dias-css',
    categoria: 'animaciones',
    tecnica: { es: "Transformaciones 3D, animaciones por fotogramas, generación de reglas con Sass.", en: "3D transforms, keyframe animations, rule generation with Sass." },
    componente: <Dia58 />,
    fondoFijo: 'oscuro',
  },
  {
    slug: 'animacion-59',
    titulo: { es: "Día 59 · Transición en tiras", en: "Day 59 · Slice Transition" },
    seccion: '100-dias-css',
    categoria: 'animaciones',
    tecnica: { es: "Generación de reglas con Sass, estados de interacción, transiciones.", en: "Rule generation with Sass, interaction states, transitions." },
    componente: <Dia59 />,
    fondoFijo: 'oscuro',
  },
  {
    slug: 'animacion-60',
    titulo: { es: "Día 60 · Superficie desenfocada", en: "Day 60 · Blurry Overlay" },
    seccion: '100-dias-css',
    categoria: 'animaciones',
    tecnica: { es: "Transiciones, posicionamiento absoluto.", en: "Transitions, absolute positioning." },
    componente: <Dia60 />,
    fondoFijo: 'oscuro',
  },
  {
    slug: 'animacion-61',
    titulo: { es: "Día 61 · Transición de degradado CSS", en: "Day 61 · CSS Gradient Transition" },
    seccion: '100-dias-css',
    categoria: 'animaciones',
    tecnica: { es: "Animaciones por fotogramas, degradados, transiciones.", en: "Keyframe animations, gradients, transitions." },
    componente: <Dia61 />,
    fondoFijo: 'oscuro',
  },
  {
    slug: 'animacion-62',
    titulo: { es: "Día 62 · Tabla de precios", en: "Day 62 · Price Table" },
    seccion: '100-dias-css',
    categoria: 'animaciones',
    tecnica: { es: "Sombras compuestas, estados de interacción, transiciones.", en: "Layered shadows, interaction states, transitions." },
    componente: <Dia62 />,
    fondoFijo: 'oscuro',
  },
  {
    slug: 'animacion-63',
    titulo: { es: "Día 63 · Anillo hipnótico", en: "Day 63 · Hypnotic Ring" },
    seccion: '100-dias-css',
    categoria: 'animaciones',
    tecnica: { es: "Animaciones por fotogramas, generación de reglas con Sass, posicionamiento absoluto.", en: "Keyframe animations, rule generation with Sass, absolute positioning." },
    componente: <Dia63 />,
    fondoFijo: 'oscuro',
  },
  {
    slug: 'animacion-64',
    titulo: { es: "Día 64 · Botón con efecto", en: "Day 64 · Button" },
    seccion: '100-dias-css',
    categoria: 'animaciones',
    tecnica: { es: "Sombras compuestas, estados de interacción, transiciones.", en: "Layered shadows, interaction states, transitions." },
    componente: <Dia64 />,
    fondoFijo: 'oscuro',
  },
  {
    slug: 'animacion-65',
    titulo: { es: "Día 65 · Elipses", en: "Day 65 · Ellipses Animation" },
    seccion: '100-dias-css',
    categoria: 'animaciones',
    tecnica: { es: "Animaciones por fotogramas, generación de reglas con Sass, transiciones.", en: "Keyframe animations, rule generation with Sass, transitions." },
    componente: <Dia65 />,
    fondoFijo: 'oscuro',
  },
  {
    slug: 'animacion-66',
    titulo: { es: "Día 66 · Casilla con destello", en: "Day 66 · Sparkle Checkbox" },
    seccion: '100-dias-css',
    categoria: 'animaciones',
    tecnica: { es: "Generación de reglas con Sass, sombras compuestas, transiciones.", en: "Rule generation with Sass, layered shadows, transitions." },
    componente: <Dia66 />,
    fondoFijo: 'oscuro',
  },
  {
    slug: 'animacion-67',
    titulo: { es: "Día 67 · Sombrilla 3D", en: "Day 67 · 3D Parasol" },
    seccion: '100-dias-css',
    categoria: 'animaciones',
    tecnica: { es: "Transformaciones 3D, animaciones por fotogramas, generación de reglas con Sass.", en: "3D transforms, keyframe animations, rule generation with Sass." },
    componente: <Dia67 />,
    fondoFijo: 'oscuro',
  },
  {
    slug: 'animacion-68',
    titulo: { es: "Día 68 · Grabación", en: "Day 68 · Recording" },
    seccion: '100-dias-css',
    categoria: 'animaciones',
    tecnica: { es: "Animaciones por fotogramas, trazado de SVG animado, posicionamiento absoluto.", en: "Keyframe animations, animated SVG stroke, absolute positioning." },
    componente: <Dia68 />,
    fondoFijo: 'oscuro',
  },
  {
    slug: 'animacion-69',
    titulo: { es: "Día 69 · Ojo", en: "Day 69 · Eye" },
    seccion: '100-dias-css',
    categoria: 'animaciones',
    tecnica: { es: "Animaciones por fotogramas, estados de interacción, transiciones.", en: "Keyframe animations, interaction states, transitions." },
    componente: <Dia69 />,
    fondoFijo: 'oscuro',
  },
  {
    slug: 'animacion-70',
    titulo: { es: "Día 70 · Días del calendario", en: "Day 70 · Calendar Days" },
    seccion: '100-dias-css',
    categoria: 'animaciones',
    tecnica: { es: "Sombras compuestas, CSS Grid, estados de interacción.", en: "Layered shadows, CSS Grid, interaction states." },
    componente: <Dia70 />,
    fondoFijo: 'oscuro',
  },
  {
    slug: 'animacion-71',
    titulo: { es: "Día 71 · Teclado morse", en: "Day 71 · Morse Code Keyboard" },
    seccion: '100-dias-css',
    categoria: 'animaciones',
    tecnica: { es: "Animaciones por fotogramas, sombras compuestas, CSS Grid.", en: "Keyframe animations, layered shadows, CSS Grid." },
    componente: <Dia71 />,
    fondoFijo: 'oscuro',
  },
  {
    slug: 'animacion-72',
    titulo: { es: "Día 72 · Los anillos", en: "Day 72 · The Rings" },
    seccion: '100-dias-css',
    categoria: 'animaciones',
    tecnica: { es: "Transformaciones 3D, animaciones por fotogramas, generación de reglas con Sass.", en: "3D transforms, keyframe animations, rule generation with Sass." },
    componente: <Dia72 />,
    fondoFijo: 'oscuro',
  },
  {
    slug: 'animacion-73',
    titulo: { es: "Día 73 · Carrusel de palabras", en: "Day 73 · Word Carousel" },
    seccion: '100-dias-css',
    categoria: 'animaciones',
    tecnica: { es: "Animaciones por fotogramas, posicionamiento absoluto.", en: "Keyframe animations, absolute positioning." },
    componente: <Dia73 />,
    fondoFijo: 'oscuro',
  },
  {
    slug: 'animacion-74',
    titulo: { es: "Día 74 · Paginación", en: "Day 74 · Pagination" },
    seccion: '100-dias-css',
    categoria: 'animaciones',
    tecnica: { es: "Estados de interacción, transiciones, posicionamiento absoluto.", en: "Interaction states, transitions, absolute positioning." },
    componente: <Dia74 />,
    fondoFijo: 'oscuro',
  },
  {
    slug: 'animacion-75',
    titulo: { es: "Día 75 · Discos giratorios", en: "Day 75 · Spinning Discs" },
    seccion: '100-dias-css',
    categoria: 'animaciones',
    tecnica: { es: "Transformaciones 3D, animaciones por fotogramas, generación de reglas con Sass.", en: "3D transforms, keyframe animations, rule generation with Sass." },
    componente: <Dia75 />,
    fondoFijo: 'oscuro',
  },
  {
    slug: 'animacion-76',
    titulo: { es: "Día 76 · Clave oculta", en: "Day 76 · Reveal Key" },
    seccion: '100-dias-css',
    categoria: 'animaciones',
    tecnica: { es: "Transformaciones 3D, sombras compuestas, transiciones.", en: "3D transforms, layered shadows, transitions." },
    componente: <Dia76 />,
    fondoFijo: 'oscuro',
  },
  {
    slug: 'animacion-77',
    titulo: { es: "Día 77 · Desenfoque de movimiento", en: "Day 77 · Motion Blur" },
    seccion: '100-dias-css',
    categoria: 'animaciones',
    tecnica: { es: "Animaciones por fotogramas, generación de reglas con Sass, posicionamiento absoluto.", en: "Keyframe animations, rule generation with Sass, absolute positioning." },
    componente: <Dia77 />,
    fondoFijo: 'oscuro',
  },
  {
    slug: 'animacion-78',
    titulo: { es: "Día 78 · Sombra al pasar", en: "Day 78 · Hover Shadow" },
    seccion: '100-dias-css',
    categoria: 'animaciones',
    tecnica: { es: "Sombras compuestas, estados de interacción, transiciones.", en: "Layered shadows, interaction states, transitions." },
    componente: <Dia78 />,
    fondoFijo: 'oscuro',
  },
  {
    slug: 'animacion-79',
    titulo: { es: "Día 79 · Líneas", en: "Day 79 · Lines Animation" },
    seccion: '100-dias-css',
    categoria: 'animaciones',
    tecnica: { es: "Animaciones por fotogramas, generación de reglas con Sass, posicionamiento absoluto.", en: "Keyframe animations, rule generation with Sass, absolute positioning." },
    componente: <Dia79 />,
    fondoFijo: 'oscuro',
  },
  {
    slug: 'animacion-80',
    titulo: { es: "Día 80 · Acordeón con Flexbox", en: "Day 80 · Flexbox Accordeon" },
    seccion: '100-dias-css',
    categoria: 'animaciones',
    tecnica: { es: "Estados de interacción, transiciones, posicionamiento absoluto.", en: "Interaction states, transitions, absolute positioning." },
    componente: <Dia80 />,
    fondoFijo: 'oscuro',
  },
  {
    slug: 'animacion-81',
    titulo: { es: "Día 81 · Pelota que rebota", en: "Day 81 · Jumping Ball" },
    seccion: '100-dias-css',
    categoria: 'animaciones',
    tecnica: { es: "Animaciones por fotogramas, curvas de aceleración propias, posicionamiento absoluto.", en: "Keyframe animations, custom easing curves, absolute positioning." },
    componente: <Dia81 />,
    fondoFijo: 'oscuro',
  },
  {
    slug: 'animacion-82',
    titulo: { es: "Día 82 · Botón de acción", en: "Day 82 · Action Button" },
    seccion: '100-dias-css',
    categoria: 'animaciones',
    tecnica: { es: "Transformaciones 3D, sombras compuestas, transiciones.", en: "3D transforms, layered shadows, transitions." },
    componente: <Dia82 />,
    fondoFijo: 'oscuro',
  },
  {
    slug: 'animacion-83',
    titulo: { es: "Día 83 · Velocidad de la luz", en: "Day 83 · Warp Drive" },
    seccion: '100-dias-css',
    categoria: 'animaciones',
    tecnica: { es: "Animaciones por fotogramas, curvas de aceleración propias, posicionamiento absoluto.", en: "Keyframe animations, custom easing curves, absolute positioning." },
    componente: <Dia83 />,
    fondoFijo: 'oscuro',
  },
  {
    slug: 'animacion-84',
    titulo: { es: "Día 84 · Portada de libro", en: "Day 84 · Book Cover" },
    seccion: '100-dias-css',
    categoria: 'animaciones',
    tecnica: { es: "Degradados, sombras compuestas, posicionamiento absoluto.", en: "Gradients, layered shadows, absolute positioning." },
    componente: <Dia84 />,
    fondoFijo: 'oscuro',
  },
  {
    slug: 'animacion-85',
    titulo: { es: "Día 85 · Algo", en: "Day 85 · It's something" },
    seccion: '100-dias-css',
    categoria: 'animaciones',
    tecnica: { es: "Animaciones por fotogramas, CSS Grid, posicionamiento absoluto.", en: "Keyframe animations, CSS Grid, absolute positioning." },
    componente: <Dia85 />,
    fondoFijo: 'oscuro',
  },
  {
    slug: 'animacion-86',
    titulo: { es: "Día 86 · Péndulo de Newton", en: "Day 86 · Newton's Cradle" },
    seccion: '100-dias-css',
    categoria: 'animaciones',
    tecnica: { es: "Animaciones por fotogramas, posicionamiento absoluto, formas circulares.", en: "Keyframe animations, absolute positioning, circular shapes." },
    componente: <Dia86 />,
    fondoFijo: 'oscuro',
  },
  {
    slug: 'animacion-87',
    titulo: { es: "Día 87 · Rubí", en: "Day 87 · Ruby" },
    seccion: '100-dias-css',
    categoria: 'animaciones',
    tecnica: { es: "Animaciones por fotogramas, sombras compuestas, posicionamiento absoluto.", en: "Keyframe animations, layered shadows, absolute positioning." },
    componente: <Dia87 />,
    fondoFijo: 'oscuro',
  },
  {
    slug: 'animacion-88',
    titulo: { es: "Día 88 · Vela", en: "Day 88 · Candle" },
    seccion: '100-dias-css',
    categoria: 'animaciones',
    tecnica: { es: "Animaciones por fotogramas, sombras compuestas, posicionamiento absoluto.", en: "Keyframe animations, layered shadows, absolute positioning." },
    componente: <Dia88 />,
    fondoFijo: 'oscuro',
  },
  {
    slug: 'animacion-89',
    titulo: { es: "Día 89 · Patrón animado", en: "Day 89 · Animated Pattern" },
    seccion: '100-dias-css',
    categoria: 'animaciones',
    tecnica: { es: "Animaciones por fotogramas, CSS Grid, posicionamiento absoluto.", en: "Keyframe animations, CSS Grid, absolute positioning." },
    componente: <Dia89 />,
    fondoFijo: 'oscuro',
  },
  {
    slug: 'animacion-90',
    titulo: { es: "Día 90 · Trampa para el cursor", en: "Day 90 · Mouse Trap" },
    seccion: '100-dias-css',
    categoria: 'animaciones',
    tecnica: { es: "Transiciones, posicionamiento absoluto.", en: "Transitions, absolute positioning." },
    componente: <Dia90 />,
    fondoFijo: 'oscuro',
  },
  {
    slug: 'animacion-91',
    titulo: { es: "Día 91 · Cargador de cubos", en: "Day 91 · Cube Loader" },
    seccion: '100-dias-css',
    categoria: 'animaciones',
    tecnica: { es: "Transformaciones 3D, animaciones por fotogramas, sombras compuestas.", en: "3D transforms, keyframe animations, layered shadows." },
    componente: <Dia91 />,
    fondoFijo: 'oscuro',
  },
  {
    slug: 'animacion-92',
    titulo: { es: "Día 92 · Puntos danzantes", en: "Day 92 · Dancing Points" },
    seccion: '100-dias-css',
    categoria: 'animaciones',
    tecnica: { es: "Animaciones por fotogramas, posicionamiento absoluto, formas circulares.", en: "Keyframe animations, absolute positioning, circular shapes." },
    componente: <Dia92 />,
    fondoFijo: 'oscuro',
  },
  {
    slug: 'animacion-93',
    titulo: { es: "Día 93 · Candado", en: "Day 93 · Padlock" },
    seccion: '100-dias-css',
    categoria: 'animaciones',
    tecnica: { es: "Sombras compuestas, estados de interacción, transiciones.", en: "Layered shadows, interaction states, transitions." },
    componente: <Dia93 />,
    fondoFijo: 'oscuro',
  },
  {
    slug: 'animacion-94',
    titulo: { es: "Día 94 · Árbol", en: "Day 94 · Tree" },
    seccion: '100-dias-css',
    categoria: 'animaciones',
    tecnica: { es: "Animaciones por fotogramas, posicionamiento absoluto.", en: "Keyframe animations, absolute positioning." },
    componente: <Dia94 />,
    fondoFijo: 'oscuro',
  },
  {
    slug: 'animacion-95',
    titulo: { es: "Día 95 · Cerdito", en: "Day 95 · Pig" },
    seccion: '100-dias-css',
    categoria: 'animaciones',
    tecnica: { es: "Animaciones por fotogramas, posicionamiento absoluto, formas circulares.", en: "Keyframe animations, absolute positioning, circular shapes." },
    componente: <Dia95 />,
    fondoFijo: 'oscuro',
  },
  {
    slug: 'animacion-96',
    titulo: { es: "Día 96 · Cuadrado saltarín", en: "Day 96 · Jumping Square" },
    seccion: '100-dias-css',
    categoria: 'animaciones',
    tecnica: { es: "Animaciones por fotogramas, sombras compuestas, curvas de aceleración propias.", en: "Keyframe animations, layered shadows, custom easing curves." },
    componente: <Dia96 />,
    fondoFijo: 'oscuro',
  },
  {
    slug: 'animacion-97',
    titulo: { es: "Día 97 · Marioneta", en: "Day 97 · Puppet" },
    seccion: '100-dias-css',
    categoria: 'animaciones',
    tecnica: { es: "Animaciones por fotogramas, posicionamiento absoluto, formas circulares.", en: "Keyframe animations, absolute positioning, circular shapes." },
    componente: <Dia97 />,
    fondoFijo: 'oscuro',
  },
  {
    slug: 'animacion-98',
    titulo: { es: "Día 98 · Plopp", en: "Day 98 · Plopp" },
    seccion: '100-dias-css',
    categoria: 'animaciones',
    tecnica: { es: "Animaciones por fotogramas, curvas de aceleración propias, posicionamiento absoluto.", en: "Keyframe animations, custom easing curves, absolute positioning." },
    componente: <Dia98 />,
    fondoFijo: 'oscuro',
  },
  {
    slug: 'animacion-99',
    titulo: { es: "Día 99 · Círculo-cuadrado", en: "Day 99 · Circle-Square" },
    seccion: '100-dias-css',
    categoria: 'animaciones',
    tecnica: { es: "Sombras compuestas, transiciones, posicionamiento absoluto.", en: "Layered shadows, transitions, absolute positioning." },
    componente: <Dia99 />,
    fondoFijo: 'oscuro',
  },
  {
    slug: 'animacion-100',
    titulo: { es: "Día 100 · Luz de neón", en: "Day 100 · Neon Light" },
    seccion: '100-dias-css',
    categoria: 'animaciones',
    tecnica: { es: "Animaciones por fotogramas, sombras compuestas, posicionamiento absoluto.", en: "Keyframe animations, layered shadows, absolute positioning." },
    componente: <Dia100 />,
    fondoFijo: 'oscuro',
  },
  {
    slug: 'campo-01',
    titulo: { es: "Campo con leyenda flotante", en: "Field with floating legend" },
    seccion: 'componentes',
    categoria: 'inputs',
    tecnica: { es: "Fieldset con legend que cambia de color al enfocar.", en: "Fieldset with a legend that recolours on focus." },
    componente: <InputType01 placeholder="Correo electrónico" />,
    alto: 140,
  },
  {
    slug: 'campo-02',
    titulo: { es: "Campo con borde animado", en: "Field with animated border" },
    seccion: 'componentes',
    categoria: 'inputs',
    tecnica: { es: "Transición del borde y la etiqueta según el estado de foco.", en: "Border and label transition driven by focus state." },
    componente: <InputType02 placeholder="Correo electrónico" />,
    alto: 140,
  },
  {
    slug: 'campo-03',
    titulo: { es: "Campo con etiqueta ascendente", en: "Field with rising label" },
    seccion: 'componentes',
    categoria: 'inputs',
    tecnica: { es: "Etiqueta que sube al escribir, sin JavaScript de posicionamiento.", en: "Label that rises while typing, with no positioning JavaScript." },
    componente: <InputType03 placeholder="Correo electrónico" />,
    alto: 140,
  },
  {
    slug: 'campo-04',
    titulo: { es: "Campo con subrayado", en: "Underlined field" },
    seccion: 'componentes',
    categoria: 'inputs',
    tecnica: { es: "Subrayado que crece desde el centro al enfocar.", en: "Underline growing from the centre on focus." },
    componente: <InputType04 placeholder="Correo electrónico" />,
    alto: 140,
  },
  {
    slug: 'campo-05',
    titulo: { es: "Campo con título", en: "Field with title" },
    seccion: 'componentes',
    categoria: 'inputs',
    tecnica: { es: "Medidas propagadas como variables CSS para que dos instancias no se pisen.", en: "Measurements passed as CSS variables so two instances do not clash." },
    componente: <InputType05 placeholder="Correo electrónico" titulo="Nombre" />,
    alto: 140,
  },
  {
    slug: 'campo-06',
    titulo: { es: "Campo con icono", en: "Field with icon" },
    seccion: 'componentes',
    categoria: 'inputs',
    tecnica: { es: "Icono posicionado dentro del campo sin romper el área de clic.", en: "Icon positioned inside the field without breaking the click area." },
    componente: <InputType06 placeholder="Correo electrónico" />,
    alto: 140,
  },
  {
    slug: 'boton-01',
    titulo: { es: "Botón de envío animado", en: "Animated submit button" },
    seccion: 'componentes',
    categoria: 'botones',
    tecnica: { es: "Transición de fondo y desplazamiento del contenido al pasar el cursor.", en: "Background transition and content shift on hover." },
    componente: <ButtonType01 />,
    alto: 140,
  },
  {
    slug: 'boton-02',
    titulo: { es: "Botón con relleno progresivo", en: "Progressive fill button" },
    seccion: 'componentes',
    categoria: 'botones',
    tecnica: { es: "Relleno que avanza desde un borde usando pseudoelementos.", en: "Fill advancing from one edge using pseudo-elements." },
    componente: <ButtonType02 />,
    alto: 140,
  },
  {
    slug: 'formulario-autenticacion',
    titulo: { es: 'Pantalla de autenticación', en: 'Authentication screen' },
    seccion: 'pantallas',
    categoria: 'formularios',
    tecnica: {
      es: 'Panel deslizante entre inicio de sesión y registro, con validación en cliente mediante react-hook-form y zod.',
      en: 'Sliding panel between sign-in and sign-up, with client-side validation via react-hook-form and zod.',
    },
    componente: <LoginDemo />,
    alto: 620,
  },
  {
    slug: "card-01",
    titulo: { es: "Producto con inclinación 3D", en: "Product card with 3D tilt" },
    seccion: 'cards',
    categoria: 'cards',
    tecnica: { es: "Transformaciones 3D según la posición del cursor y brillo radial pasado como variable CSS.", en: "3D transforms driven by cursor position, with a radial highlight passed as a CSS variable." },
    componente: (
      <Carrusel>
        {zapatillas.map((elemento) => (
          <CardTipo01 key={elemento.id} elemento={elemento} />
        ))}
      </Carrusel>
    ),
    alto: 320,
  },
  {
    slug: "card-02",
    titulo: { es: "Ficha de persona con secciones", en: "Profile card with sections" },
    seccion: 'cards',
    categoria: 'cards',
    tecnica: { es: "Panel deslizante entre secciones y color de la ficha propagado como variable CSS.", en: "Sliding panel between sections, with the card colour propagated as a CSS variable." },
    componente: (
      <Carrusel>
        {empleados.map((elemento) => (
          <CardTipo02 key={elemento.nombre} ficha={elemento} />
        ))}
      </Carrusel>
    ),
    alto: 320,
  },
  {
    slug: "card-03",
    titulo: { es: "Perfil sobre imagen", en: "Profile over an image" },
    seccion: 'cards',
    categoria: 'cards',
    tecnica: { es: "Zoom del fondo al pasar el cursor, capa de contraste y acciones al pie.", en: "Background zoom on hover, contrast layer and footer actions." },
    componente: (
      <Carrusel>
        {perfilesRed.map((elemento) => (
          <CardTipo03 key={elemento.nombre} perfil={elemento} />
        ))}
      </Carrusel>
    ),
    alto: 320,
  },
  {
    slug: "card-04",
    titulo: { es: "Producto con ficha desplegable", en: "Product card with unfolding details" },
    seccion: 'cards',
    categoria: 'cards',
    tecnica: { es: "Círculo que se expande y panel de tallas y colores que aparece al pasar el cursor.", en: "Expanding circle and a sizes-and-colours panel revealed on hover." },
    componente: (
      <Carrusel>
        {zapatillas.map((elemento) => (
          <CardTipo04 key={elemento.id} producto={elemento} />
        ))}
      </Carrusel>
    ),
    alto: 320,
  },
  {
    slug: "card-05",
    titulo: { es: "Producto de tienda con variantes", en: "Store product with variants" },
    seccion: 'cards',
    categoria: 'cards',
    tecnica: { es: "Selector de color que intercambia la imagen, valoración en SVG y texto recortado a dos líneas.", en: "Colour picker swapping the image, SVG rating and text clamped to two lines." },
    componente: (
      <Carrusel>
        {productos.map((elemento) => (
          <CardTipo05 key={elemento.nombre} producto={elemento} />
        ))}
      </Carrusel>
    ),
    alto: 320,
  },
  {
    slug: "card-06",
    titulo: { es: "Lámina de coleccionable", en: "Collectible poster" },
    seccion: 'cards',
    categoria: 'cards',
    tecnica: { es: "Perspectiva 3D: el fondo se tumba y la figura sale del marco al pasar el cursor.", en: "3D perspective: the backdrop tilts and the figure steps out of the frame on hover." },
    componente: (
      <Carrusel>
        {coleccionables.map((elemento) => (
          <CardTipo06 key={elemento.imgFondo} pieza={elemento} />
        ))}
      </Carrusel>
    ),
    alto: 340,
  },
  {
    slug: "card-07",
    titulo: { es: "Destino de viaje", en: "Travel destination" },
    seccion: 'cards',
    categoria: 'cards',
    tecnica: { es: "Panel que asciende, subrayado que entra desde un lado y texto revelado con retardo.", en: "Rising panel, underline sliding in from one side and a delayed text reveal." },
    componente: (
      <Carrusel>
        {destinos.map((elemento) => (
          <CardTipo07 key={elemento.lugar} destino={elemento} />
        ))}
      </Carrusel>
    ),
    alto: 320,
  },
  {
    slug: "card-08",
    titulo: { es: "Promoción con óvalo", en: "Promo card with an oval" },
    seccion: 'cards',
    categoria: 'cards',
    tecnica: { es: "Óvalo que sube al pasar el cursor y franja de descuento que responde al rótulo.", en: "Oval rising on hover and a discount band reacting to the label." },
    componente: (
      <Carrusel>
        {promociones.map((elemento) => (
          <CardTipo08 key={elemento.img} promocion={elemento} />
        ))}
      </Carrusel>
    ),
    alto: 320,
  },
];

/** Piezas de una sección, en el orden en que se registraron. */
export function piezasPorSeccion(seccion: PiezaLab['seccion']): PiezaLab[] {
  return piezasLab.filter((pieza) => pieza.seccion === seccion);
}

/** Cuántas piezas tiene cada sección, para el índice del Laboratorio. */
export function conteoPorSeccion(): Record<string, number> {
  return piezasLab.reduce<Record<string, number>>((acumulado, pieza) => {
    acumulado[pieza.seccion] = (acumulado[pieza.seccion] ?? 0) + 1;
    return acumulado;
  }, {});
}
